...
class ResourceFarmerState:
    ...
    def represent(self):
        if self._representation is None:
            ...
            if PlayedCharacterManager().inventoryWeightMax != 0:
                state_repr.append(PlayedCharacterManager().inventoryWeight / PlayedCharacterManager().inventoryWeightMax)
            else:
                state_repr.append(-1)
            ...
            for jobId in self.jobImportance:
                ...
                if job:
                    ...
                    state_repr.append((job.jobXP - job.jobXpLevelFloor) / (job.jobXpNextLevelFloor - job.jobXpLevelFloor))
            ...
            for i in range(self.MAX_RESOURCES):
                if i < len(self.resources):
                    ...
                    if self.resources[i].uid in self.lastTimeResourceCollected:
                        timeSinceLastCollect = (time.time() - self.lastTimeResourceCollected[self.resources[i].uid]) // 60
                    else:
            ...
            for i in range(self.MAX_NEIGHBORS):
                if i < len(self.outgoingEdges):
                    ...
                    state_repr.append(self.lastTimeVertexVisited.get(dstV, -1))
                else:
    ...
    def getValidAction(self, q_values):
        ...
        num_outgoing_edges = len(self.outgoingEdges)

        # Mark the valid resource actions as 1
        for resource_index in range(num_resources):
            r = self.resources[resource_index]
            if r not in self.forbiddenActions and r.canFarm(self.jobFilter):
                valid_actions_mask[resource_index] = 1
                
        # Mark the valid vertex actions as 1
        valid_actions_mask[self.MAX_RESOURCES:self.MAX_RESOURCES + num_outgoing_edges] = 1
        ...
        assert 0 <= res < num_resources or self.MAX_RESOURCES <= res < self.MAX_RESOURCES + num_outgoing_edges
    ...
    def getRandomValidAction(self):
        ...
        valid_actions = []
        num_resources = min(len(self.resources), self.MAX_RESOURCES)
        vertex_visits = [self.nbrVertexVisites.get(edge.dst, 0) for edge in self.outgoingEdges]
        ...
        valid_actions.append(self.MAX_RESOURCES + np.argmin(vertex_visits))
        
        # Randomly choose an action from valid actions
        res = np.random.choice(valid_actions)
        assert 0 <= res < num_resources or self.MAX_RESOURCES <= res < self.MAX_RESOURCES + len(self.outgoingEdges)
        assert res < self.ACTION_SIZE
        return res
