...
cert = AccountManager.getCert(accountId)
key = AccountManager.get_apikey(accountId)
...
def onShutDown():
    ...
    bot.shutdown(DisconnectionReasonEnum.WANTED_SHUTDOWN, f"Character '{character.login}' shutdowned.")
bot.addShutDownListener(onShutDown)
bot.setConfig(key, cert["id"], cert["hash"], session, CharacterRoleEnum.LEADER, character)
bot.start()
bot.join()